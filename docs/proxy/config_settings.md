...
| LITELLM_ENABLE_HTTP2 | If true, opts in to HTTP/2 for outbound LLM requests (equivalent to `litellm.enable_http2 = True`). Requires the `h2` package. Off by default; falls back to HTTP/1.1 over aiohttp when unset.
